...
// module configs
$cfg = loadConfigurations('contact');
...
if(check($_POST['submit'])) {
	try {
		if(!check($_POST['contact_email'])) throw new Exception(lang('error_41',true));
		if(!check($_POST['contact_message'])) throw new Exception(lang('error_41',true));
